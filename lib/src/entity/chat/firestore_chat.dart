...
class FirestoreChat with ChatStruct, ToReferenceMixin implements Chat {
  FirestoreChat factory FirestoreChat.fromDocument(DocumentSnapshot document) {
    ...
    assert(maybeLastChatMessage == null || maybeLastChatMessage is DocumentReference);

    final members = (maybeMembers as Iterable)
      .map((maybeMember) =>
        UserReference.fromFirestoreDocumentReference(maybeMember),
      );
    final lastChatMessage = maybeLastChatMessage == null ? null : ChatMessageReference.fromFirestoreDocumentReference(maybeLastChatMessage);
